...
Sistemas de IA devem seguir o Princípio do Privilégio Mínimo Agêntico (*Least Privilege*):
- Os Agentes Orquestrados pela CLI, ou rodando no Jenkins, atuam num *AgentFS* ou containers Docker limpos de rotina, sem acesso como Root para evitar danos de manipulação destrutiva auto-referenciada no PC do humano (Drop file system).
