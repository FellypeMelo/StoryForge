...
fn test_cross_feature_integration() {
    ...
    character_repo
        .create_character(&char_a)
        .expect("Failed to create char_a");
    character_repo
        .create_character(&char_b)
        .expect("Failed to create char_b");
    ...
    let characters = character_repo
        .list_characters_by_project(&project.id)
        .unwrap();
    assert_eq!(characters.len(), 2);

    let relationships = relationship_repo
        .list_global_relationships(&project.id)
        .unwrap();
