...
describe("WorldbuildingPipeline", () => {
  ...
  it("should execute all 4 steps of CAD in sequence", async () => {
    ...
    // Step 2: Economy
    (llmPort.complete as any).mockResolvedValueOnce({
      text: "Resource scarcity based on magic...",
    });
    // Step 3: Sociology
  ...
  it("should throw error if repository fails to save", async () => {
    ...
    (worldRuleRepository.save as any).mockResolvedValue({
      success: false,
      error: new Error("DB Error"),
    });

    await expect(pipeline.run(projectId, premise)).rejects.toThrow(
      "Failed to save world rule: DB Error",
    );
  });
});
