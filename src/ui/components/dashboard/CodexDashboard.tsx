...
export function CodexDashboard({ projectId, bookId, onBack }: CodexDashboardProps) {
  ...
  const loadData = async () => {
    ...
    try {
      if (activeTab === "characters") {
        ...
        const command = scope === "book" ? "list_timeline_events_by_book" : "list_global_timeline_events";
        const params = scope === "book" ? { bookId } : { projectId: currentProjectId };
        const data = await invoke<any[]>(command, params);
        setEvents(data.map(d => TimelineEvent.create({
          ...d,
          id: { value: d.id },
          projectId: { value: d.project_id },
          bookId: d.book_id ? { value: d.book_id } : undefined,
        ...
        const command = scope === "book" ? "list_relationships_by_book" : "list_global_relationships";
        const params = scope === "book" ? { bookId } : { projectId: currentProjectId };
        const data = await invoke<any[]>(command, params);
        setRelationships(data.map(d => Relationship.create({
          ...d,
          id: { value: d.id },
          projectId: { value: d.project_id },
          bookId: d.book_id ? { value: d.book_id } : undefined,
        ...
        const command = scope === "book" ? "list_blacklist_entries_by_book" : "list_global_blacklist_entries";
        const params = scope === "book" ? { bookId } : { projectId: currentProjectId };
        const data = await invoke<any[]>(command, params);
        setEntries(data.map(d => BlacklistEntry.create({
          ...d,
          id: { value: d.id },
          projectId: { value: d.project_id },
          bookId: d.book_id ? { value: d.book_id } : undefined,
  ...
  const handleSaveCharacter = async (char: Character) => {
    try {
      ...
      const created = await invoke<{ id: string }>("create_character", { 
        projectId: currentProjectId, 
        bookId: scope === "book" ? bookId : null,
        name: charData.name 
      });
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border-subtle pb-6">
        <div>
          ...
          <p className="text-text-muted text-sm font-sans">
            Guia de sabedoria. <span className="opacity-50 text-xs">Projeto: {projectId.slice(0, 8)} | Livro: {bookId.slice(0, 8)}</span>
          </p>
        </div>
      ...
      <main className="min-h-[400px]">
        {isSearching ? (
          ...
          <>
            ...
            {activeTab === "relationships" && <RelationshipList relationships={relationships} onCreateNew={() => {}} />}
            {activeTab === "blacklist" && <BlacklistList entries={entries} onCreateNew={() => {}} />}
          </>
        )}
      </main>
        ...
        <div className="h-full flex flex-col">
          {editingCharacter && (
            <div className="p-4 border-b border-border-subtle flex justify-end gap-2 bg-bg-hover/50">
              {editingCharacter.bookId ? (
                <button 
                  onClick={() => handleMoveToProject(editingCharacter.id.value)}
                ...
                <button 
                  onClick={() => handleMoveToBook(editingCharacter.id.value)}
